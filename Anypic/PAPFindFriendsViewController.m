...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Featured Users";
    ...
    if ([MFMailComposeViewController canSendMail] || [MFMessageComposeViewController canSendText]) {
        ...
        NSString *inviteString = NSLocalizedString(@"Check out these users", @"Check out these users");
        ...
        //[inviteLabel setBackgroundColor:[UIColor darkGrayColor]];
...
- (PFQuery *)queryForTable {
    ...
    [parseEmployees removeObject:[[PFUser currentUser] objectForKey:kPAPUserDisplayNameKey]];
    PFQuery *parseEmployeeQuery = [PFUser query];
    [parseEmployeeQuery whereKey:kPAPUserDisplayNameKey containedIn:parseEmployees];
    
    PFQuery *query = [PFQuery orQueryWithSubqueries:[NSArray arrayWithObjects: parseEmployeeQuery, nil]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath object:(PFObject *)object {
    ...
    [cell.photoLabel setText:@"0 listings"];
    //[cell.photoLabel setBackgroundColor:[UIColor darkGrayColor]];
    [cell.photoLabel setTextColor:[UIColor whiteColor]];
    ...
    if (attributes) {
        ...
        [cell.photoLabel setText:[NSString stringWithFormat:@"%@ listing%@", number, [number intValue] == 1 ? @"": @"s"]];
    } else {
        @synchronized(self) {
            ...
            if (!outstandingCountQueryStatus) {
                ...
                [photoNumQuery countObjectsInBackgroundWithBlock:^(int number, NSError *error) {
                    ...
                    [actualCell.photoLabel setText:[NSString stringWithFormat:@"%d listing%@", number, number == 1 ? @"" : @"s"]];
                }];
            };
        }
    }
