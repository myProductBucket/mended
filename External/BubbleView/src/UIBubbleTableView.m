...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.section >= [self.bubbleSection count])
    {
        ...
        if (cell == nil)
#if !__has_feature(objc_arc)
            cell = [[[UIBubbleTypingTableViewCell alloc] init] autorelease];
#else
    ...
    if (indexPath.row == 0)
    {
        ...
        if (cell == nil)
#if !__has_feature(objc_arc)
            cell = [[[UIBubbleHeaderTableViewCell alloc] init] autorelease];
#else
...
{
    if (self.typingBubble == NSBubbleTypingTypeLoading && scrollView.contentOffset.y + scrollView.frame.size.height > scrollView.contentSize.height - 10) {
        if ([self.bubbleDelegate respondsToSelector:@selector(bubbleTableViewShouldLoadMore:)]) {
            [self.bubbleDelegate bubbleTableViewShouldLoadMore:self];
        }
    }
}
